                                    ...
                                    <select style="width: 182px" name="gibbonCourseIDCopyTo" id="gibbonCourseIDCopyTo">
                                        ...
                                        try {
                                            ...
                                            if ($gibbonSchoolYearIDNext != false) {
                                                ...
                                                $sqlWhere = ' OR gibbonCourse.academic_year=:gibbonSchoolYearIDNext';
                                            }
                                            if ($highestAction == 'Unit Planner_all') {
                                                $sqlSelect="SELECT gibbonCourse.gibbonCourseID, gibbonCourse.nameShort AS course, gibbonSchoolYear.name AS year FROM gibbonCourse JOIN gibbonDepartment ON (gibbonCourse.gibbonDepartmentID=gibbonDepartment.gibbonDepartmentID) JOIN gibbonSchoolYear ON (gibbonCourse.academic_year=gibbonSchoolYear.gibbonSchoolYearID) WHERE (gibbonCourse.academic_year=:gibbonSchoolYearID".$sqlWhere.") ORDER BY sequenceNumber, gibbonCourse.nameShort" ;
                                            }
                                            else {
                                                $dataSelect['gibbonPersonID'] = $_SESSION[$guid]["gibbonPersonID"];
                                                $sqlSelect="SELECT gibbonCourse.gibbonCourseID, gibbonCourse.nameShort AS course, gibbonSchoolYear.name AS year FROM gibbonCourse JOIN gibbonDepartment ON (gibbonCourse.gibbonDepartmentID=gibbonDepartment.gibbonDepartmentID) JOIN gibbonDepartmentStaff ON (gibbonDepartmentStaff.gibbonDepartmentID=gibbonDepartment.gibbonDepartmentID) JOIN gibbonSchoolYear ON (gibbonCourse.academic_year=gibbonSchoolYear.gibbonSchoolYearID) WHERE gibbonDepartmentStaff.gibbonPersonID=:gibbonPersonID AND (role='Coordinator' OR role='Assistant Coordinator' OR role='Teacher (Curriculum)') AND (gibbonCourse.academic_year=:gibbonSchoolYearID".$sqlWhere.") ORDER BY sequenceNumber, gibbonCourse.nameShort" ;
                                            }
