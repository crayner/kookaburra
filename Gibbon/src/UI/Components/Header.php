...
class Header
{
    ...
    public function getNotificationTray($cacheLoad)
    {
        ...
        if (isset($_SESSION[$guid]['username']) != false) {
            ...
            if ($resultNotifications->rowCount() > 0) {
                $return .= "<a class='inline-block relative mr-4' title='".__('Notifications')."' href='".$_SESSION[$guid]['absoluteURL']."/notifications/manage/'><span class='badge -mr-2 right-0'>".$resultNotifications->rowCount()."</span><img class='minorLinkIcon' style='margin-left: 2px; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['gibbonThemeName']."/img/notifications.png'></a>";
            } else {
                $return .= "<a class='inactive inline-block relative mr-4' title='".__('Notifications')."' href='".$_SESSION[$guid]['absoluteURL']."/notifications/manage/'><img class='minorLinkIcon' style='margin-left: 2px; opacity: 0.2; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['gibbonThemeName']."/img/notifications.png'></a>";
            }
            $return .= '</div>';
        }
    ...
    public function getMinorLinks($cacheLoad)
    {
        ...
        if (isset($_SESSION[$guid]['username']) == false) {
            ...
            $return .= $name.' . ';
            $return .= "<a class='link-white' href='".$this->session->get('absoluteURL')."/logout/'>".__('Logout')."</a> . <a class='link-white' href='".$this->session->get('absoluteURL')."/user/admin/preferences/'>".__('Preferences').'</a>';
