...
class Core
{
     ...
     * Construct
     */
    public function __construct(string $directory, bool $installed = false)
    {
...
//        $this->locale->setStringReplacementList($db);
    ...
    {
        return (file_exists($this->basePath . '/config.php') && filesize($this->basePath . '/config.php') > 0) || $this->installed;
    }
    ...
    protected function loadConfigFromFile($configFilePath)
    {
        ...
        if (!isset($this->config['databasePort'])) $this->config['databasePort'] = '';
